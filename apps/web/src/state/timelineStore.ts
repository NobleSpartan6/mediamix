...
const pruneTracks = (tracks: Track[], clips: Record<string, Clip>): Track[] => {
  const maxLane = Math.max(-1, ...Object.values(clips).map((c) => c.lane))
  return tracks.slice(0, Math.max(0, maxLane + 1))
}
...
export const useTimelineStore = create<TimelineState>((set) => ({
  ...
  addClip: (clipInput) => {
    ...
    set((state) => {
      ...
      let tracks = ensureTracks(state.tracks, newClip.lane)
      tracks = pruneTracks(tracks, clipsById)
