...
export const InteractiveClip: React.FC<InteractiveClipProps> = React.memo(({ clip, pixelsPerSecond, type }) => {
  ...
  const asset = useMediaStore(
    React.useCallback((s) => (clip.assetId ? s.assets[clip.assetId] : undefined), [clip.assetId]),
  )
  ...
  const laneHeights = React.useMemo(() => tracks.map((t) => (t.type === 'video' ? 48 : 32)), [tracks])
  ...
  const clipTrackType = React.useMemo(() => laneTypes[clip.lane] ?? type, [laneTypes, clip.lane, type])
  ...
  const globalTimes = React.useMemo(
    () => allClips.filter((c) => c.id !== clip.id).flatMap((c) => [c.start, c.end]),
    [allClips, clip.id],
  )
  ...
  const applyTransform = React.useCallback((x: number, y = 0, newWidthPx?: number) => {
  ...
  const onDragEnd = React.useCallback(() => {
    ...
    const finalLane = laneTypes[lane] === clipTrackType ? lane : clip.lane
    ...
    if (selectedIds.length > 1) {
      ...
      for (const id of selectedIds) {
        ...
        if (newStart < 0 || laneHasCollision(c.lane, newStart, newEnd, id)(state)) {
          invalid = true
          break
        }
      }
