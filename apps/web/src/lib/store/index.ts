...
const useMotifStore = create<MotifState>((set) => ({
  ...
  setFileInfo: (info: Partial<FileInfo>) =>
    set((state) => ({
      fileInfo: { ...state.fileInfo, ...info },
    })),

  setIsFileLoading: (loading: boolean) =>
    set(() => ({
  ...
  addMediaAsset: (assetInput) => {
    ...
    const maxLane = Object.values(timeline.clipsById).reduce((m, c) => Math.max(m, c.lane), -1)
    const baseLane = maxLane + 1
    const duration = assetInput.metadata.duration ?? 0
    const groupId = nanoid()
    timeline.addClip({ start: 0, end: duration, lane: baseLane, assetId: id }, { trackType: 'video', groupId })
    timeline.addClip({ start: 0, end: duration, lane: baseLane + 1, assetId: id }, { trackType: 'audio', groupId })
  },

  /** Replace all media assets */
  setMediaAssets: (assets) => set(() => ({ mediaAssets: assets })),

  setBeatMarkers: (markers: BeatMarker[]) =>
    set(() => ({
      beatMarkers: markers,
    })),

  addClip: (clip: ClipSegment) =>
    set((state) => ({
      timeline: {
        ...state.timeline,
  ...
  updateClip: (id: string, updates: Partial<ClipSegment>) =>
    set((state) => ({
      timeline: {
        ...state.timeline,
        clips: state.timeline.clips.map((clip) => (clip.id === id ? { ...clip, ...updates } : clip)),
      },
    })),
  ...
  setExportStatus: (isExporting: boolean, progress = 0, error = null) =>
    set(() => ({
      isExporting,
  ...
  setIsBeatDetectionRunning: (running: boolean) =>
    set(() => ({
      isBeatDetectionRunning: running,
    })),

  setBeatDetectionError: (error: string | null) =>
    set(() => ({
      beatDetectionError: error,
    })),

  setBeatDetectionProgress: (progress: number) =>
    set(() => ({
      beatDetectionProgress: progress,
    })),

  setBeatDetectionStage: (stage: 'idle' | 'extractAudio' | 'detectBeats') =>
    set(() => ({
