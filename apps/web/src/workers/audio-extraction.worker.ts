...
async function getFFmpegInstance(): Promise<FFmpeg> {
  if (
    ffmpegInstance &&
    ((ffmpegInstance as any).isLoaded ? (ffmpegInstance as any).isLoaded() : (ffmpegInstance as any).loaded)
  ) {
...
self.onmessage = async (event: MessageEvent<WorkerIncomingMessage>) => {
  ...
  if (type === 'EXTRACT_AUDIO') {
    ...
    try {
      ...
      const ffmpegArgs = ['-threads', '8', '-i', inputFileName, '-vn', '-vsync', '2']
      ...
      const errorMessage: ErrorMessage = {
        type: 'ERROR',
        error: (error as Error).message || 'Unknown error in FFmpeg operation',
      }
